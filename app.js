...
const SUPABASE_URL = "gpxdgperbfgcdfwwiaed";
const SUPABASE_ANON_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImdweGRncGVyYmZnY2Rmd3dpYWVkIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzgxNzY2NzAsImV4cCI6MjA5Mzc1MjY3MH0.aZwaXKg3Dwz7NnqR0tF3T6G50gXG5N8yzAjBjsfNUe4";
// ======================
...
formIncidencia.addEventListener("submit", async (ev) => {
  ...
  try {
    ...
    supabaseClient.from("incidencias").insert(payload);
...
async function loadIncidencias() {
  try {
    ...
    supabaseClient.from("incidencias").select("id, created_at, aula, equipo, tipo, estado").order("created_at", { descending: true });
    const { data, error } = await supabaseClient.from("incidencias").select("id, created_at, aula, equipo, tipo, estado").order("created_at", { descending: true });
    if (error) throw error;
...
async function cerrarIncidencia(id) {
  try {
    ...
    supabaseClient.from("incidencias").update({ estado: "cerrada" }).eq("id", id);
    const { error } = await supabaseClient.from("incidencias").update({ estado: "cerrada" }).eq("id", id);
    if (error) throw error;
